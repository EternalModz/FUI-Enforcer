...
namespace FUI_Enforcer.Forms
{
    public partial class MainForm : Form
    {
        ...
        {
            if (index < 0) return;
            if (index > imageListView.Items.Count) return;
            ...
            using (MemoryStream stream = new MemoryStream(filedata, false))
            {
                ...
                imageListView.Items[index].Text = $"{index} - {imageLoaded.Width}x{imageLoaded.Height}";
                ...
                for (int i = 0; i < imagesData.Count; i++)
                {
                    ...
                    using (MemoryStream stream = new MemoryStream(imageData, false))
                    {
                        cinvk(() =>
                        {
                            ...
                            imageList.Images.Add(ImageUtils.CreateThumbnail(image, imageList.ImageSize, this.imageListView.BackColor));
        ...
        private void imageListView_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ind != -1 & imageListView.SelectedIndices.Count == 1)
            {
        ...
        {
            var selInd = imageListView.SelectedIndices.Count > 0 ? imageListView.SelectedIndices[0] : -1;
            var count = imageListView.SelectedIndices.Count - 1;
